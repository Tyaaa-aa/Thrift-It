...
if (in_array($fileActualExt, $allowed)) {
    ...
    $craftlist_id = $_POST['craftlist_id'];
    ...
    $stmt = $conn->prepare("UPDATE sp_craftlistings set craftlist_title=?, craftlist_price=?, craftlist_image=?, craftlist_desc=?, craftlist_iUserid=? where craftlist_id = $craftlist_id");

    
    
    $stmt->bind_param("sissi", $list_title, $list_price, $fileDestination, $list_description, $userid);
    $stmt->execute();
    $stmt->close();
    $conn->close();

    echo "<script type='text/javascript'>
    alert('Your Craft Listing Has Been Updated!');
